...
public class Soldier extends Unit {
    ...
    void play() throws GameActionException {
        ...
        if (rc.getPaint() < 20) target = home; // return home
        else if (closestRuin != null) target = closestRuin;
        else {
            target = rc.getLocation().translate(
                    rc.getLocation().directionTo(home).dx * -3,
                    rc.getLocation().directionTo(home).dy * -3
            );
        }
        ...
        if (rc.isActionReady() && rc.getPaint() >= 20) {
            ...
            if (paintLoc != null) {
                ...
                if (closestRuin != null &&
                    ...
                    secondary = (GameConstants.PAINT_TOWER_PATTERN &
                            (1 << (relX + 5*relY))) > 0;
                } else {
        ...
        for (MapInfo m : nearLocations) {
            ...
            if (m.getPaint() == PaintType.EMPTY) {
                ...
                if (closestRuin != null &&
                    ...
                    boolean secondary = (GameConstants.PAINT_TOWER_PATTERN &
                            (1 << (relX + 5*relY))) > 0;
