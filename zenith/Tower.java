...
//        msg |= Math.min((rc.getRoundNum()) / 10, 31) << 14; // idk
    ...
    void decodeMessages() throws GameActionException {
        ...
        for (int i = 0; i < messages.length && Clock.getBytecodesLeft() > 9000; i++) {
    ...
    MapLocation nextSpawnLocation(UnitType type) {
        MapLocation center = new MapLocation(rc.getMapWidth()/2, rc.getMapHeight()/2);
        if (rc.getRoundNum() < 30 || Math.random() > 0.1) {
            switch (rc.getRoundNum() % 5) {
                case 0:
                    if (rc.canBuildRobot(type, rc.getLocation().add(rc.getLocation().directionTo(center).rotateRight())))
                        return rc.getLocation().add(rc.getLocation().directionTo(center).rotateRight());
                    break;
                case 1:
                    if (rc.canBuildRobot(type, rc.getLocation().add(rc.getLocation().directionTo(center).rotateLeft())))
                        return rc.getLocation().add(rc.getLocation().directionTo(center).rotateLeft());
                    break;
                case 2:
                    if (rc.canBuildRobot(type, rc.getLocation().add(rc.getLocation().directionTo(center))))
                        return rc.getLocation().add(rc.getLocation().directionTo(center));
                    break;
                case 3:
                    if (rc.canBuildRobot(type, rc.getLocation().add(rc.getLocation().directionTo(center).rotateRight().rotateRight())))
                        return rc.getLocation().add(rc.getLocation().directionTo(center).rotateRight().rotateRight());
                    break;
                case 4:
                    if (rc.canBuildRobot(type, rc.getLocation().add(rc.getLocation().directionTo(center).rotateLeft().rotateLeft())))
                        return rc.getLocation().add(rc.getLocation().directionTo(center).rotateLeft().rotateLeft());
                    break;
            }
        } else if (rc.canBuildRobot(type, rc.getLocation().add(center.directionTo(rc.getLocation()))))
            return rc.getLocation().add(center.directionTo(rc.getLocation()));

        for (int i=11; i>=0; i--) {
            int id = (int)(Math.random() * 12);
            if (rc.canBuildRobot(type, spawnPlaces[id])) {
                return spawnPlaces[id];
            }
        }
