...
public abstract class Unit extends Robot {
    ...
    void turn1() throws GameActionException {
        ...
        if (rc.getRoundNum() == 2) {
            ...
            saveSRPLoc = rc.getLocation().translate(
                    rc.getLocation().directionTo(center).dx * 10,
                    rc.getLocation().directionTo(center).dy * 10);
        } else if (rc.getRoundNum() == 3) {
            MapLocation center = new MapLocation(rc.getMapWidth()/2, rc.getMapHeight()/2);
            saveSRPLoc = rc.getLocation().translate(
                    rc.getLocation().directionTo(center).rotateRight().dx * 10,
                    rc.getLocation().directionTo(center).rotateRight().dy * 10);
        } else if (rc.getRoundNum() == 4) {
            MapLocation center = new MapLocation(rc.getMapWidth()/2, rc.getMapHeight()/2);
            saveSRPLoc = rc.getLocation().translate(
                    rc.getLocation().directionTo(center).rotateLeft().dx * 10,
